...
static void SGZApplySpeed(void) {
    ...
    for (UIView *v in found) {
        ...
        if (scene) {
            [scene setValue:[NSNumber numberWithFloat:g_speedMult] forKey:@"speed"];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        st.numberOfLines = 0;
        st.text = @"SpriteKit 原生游戏——数值直写内存\n秒杀=清零 无敌=回峰值（互斥）\n变速=官方 scene.speed\n日志: Documents/sgz_cheat.log";
        [self addSubview:st];
...
- (void)vg_onSpeed:(UIButton *)s {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{ SGZApplySpeed(); });
    for (UIView *v in self.subviews) {
...
__attribute__((constructor)) static void vg_ctor() {
    @autoreleasepool {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)),
            ...
            fg_ensureButton();
            // SpriteKit 变速巡检（每秒，新场景自动应用）
            static dispatch_source_t sp = nil;
            if (!sp) {
                sp = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
                dispatch_source_set_timer(sp, DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC, 200 * NSEC_PER_MSEC);
                dispatch_source_set_event_handler(sp, ^{ SGZApplySpeed(); });
                dispatch_resume(sp);
            }
